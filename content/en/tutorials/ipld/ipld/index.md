...
The original block is actually DAG-PB format which is used to construct UnixFS file data in IPFS. If we switch to the `ipfs block get` command and provide the CID then we’ll get the raw DAG-PB bytes, but they’re not as easy to read!
Inspect the binary format of the data:

```
ipfs block get QmQ2ocFLq6d7ZiVEQfuEGEr4niJmdSscoyLkgTKRWmAEq
```
You will get illegible information, since it is raw DAG-PB bytes
...
As an example, take a look at this directory in the [IPLD github repo](https://github.com/ipld/ipld/tree/master/docs/codecs/known):
